...
type PushStatus = 'unsupported' | 'install-required' | 'available' | 'enabled' | 'blocked' | 'error';

let lastRegisteredSubscription: string | null = null;
...
export const usePushNotifications = () => {
    ...
    async function persistSubscription(role: AuthRole, subscription: PushSubscription) {
        const token = getSubscriptionToken(subscription);
        if (token === lastRegisteredSubscription) {
            return;
        }

        lastRegisteredSubscription = token;

        await api.post(endpointForRole(role), {
            token,
            platform: 'web',
            provider: 'webpush',
    ...
    async function unregisterSubscription(role: AuthRole, subscription: PushSubscription) {
        const token = getSubscriptionToken(subscription);

        await api.post(`${endpointForRole(role)}/unregister`, { token });
        await subscription.unsubscribe();

        lastRegisteredSubscription = null;
    }

    async function ensurePushRegistration(role: AuthRole) {
        if (!supportsWebPush(runtimeConfig.public.webPushVapidPublicKey)) {
            updateStatus(runtimeConfig.public.webPushVapidPublicKey, null);
            return false;
        }
    ...
    async function enablePush(role: AuthRole) {
        if (!supportsWebPush(runtimeConfig.public.webPushVapidPublicKey)) {
            updateStatus(runtimeConfig.public.webPushVapidPublicKey, null);
            return false;
        }
    ...
    async function disablePush(role: AuthRole) {
        if (!supportsWebPush(runtimeConfig.public.webPushVapidPublicKey)) {
            updateStatus(runtimeConfig.public.webPushVapidPublicKey, null);
            return false;
        }
    ...
    async function syncPushRegistration(role: AuthRole) {
        if (!supportsWebPush(runtimeConfig.public.webPushVapidPublicKey)) {
            updateStatus(runtimeConfig.public.webPushVapidPublicKey, null);
            return;
        }
