...
<UCard class='xl:min-h-[32rem]'>
        ...
        <div class='flex flex-wrap items-start justify-between gap-3'>
            <div>
                ...
                <p class='mt-1 text-xs text-slate-400'>
                    Wichtige Hinweise zu deinen Kursen und allgemeine Updates.
                </p>
            </div>
            <UButton variant='ghost' size='xs' to='/customer/notifications'>
                Alle
            </UButton>
        </div>
    </template>
    ...
    <div v-else class='divide-y divide-slate-100'>
        ...
        >
            ...
            <p v-if='notification.isGlobal' class='mt-0.5 text-xs text-amber-700'>
                Alle Kurse
            </p>
            <p v-else-if='notification.courses.length > 0' class='mt-0.5 text-xs text-komm-700'>
                {{ formatNotificationCourses(notification.courses) }}
            </p>
