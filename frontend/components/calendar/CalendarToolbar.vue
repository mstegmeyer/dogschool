<template>
  <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="flex items-center gap-2">
      ...
      <UButton data-testid="calendar-prev" icon="i-heroicons-chevron-left" variant="ghost" size="sm" @click="emit('prev')" />
      <span class="min-w-[100px] text-center text-sm font-medium text-slate-600 sm:min-w-[160px]">
        {{ rangeLabel }}
      </span>
      <UButton data-testid="calendar-next" icon="i-heroicons-chevron-right" variant="ghost" size="sm" @click="emit('next')" />
      <UButton data-testid="calendar-today" variant="outline" size="sm" label="Heute" class="ml-1" @click="emit('today')" />
    </div>
  </div>
</template>
