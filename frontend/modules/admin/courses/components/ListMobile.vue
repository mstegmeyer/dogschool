...
<div class='space-y-3 md:hidden'>
    ...
    >
        <div class='flex items-start justify-between gap-3'>
            <div class='min-w-0'>
                ...
                <p class='mt-1 text-sm text-slate-500'>
                    {{ dayName(course.dayOfWeek) }} · {{ course.startTime }} – {{ course.endTime }}
                </p>
            </div>
        ...
        <div class='mt-3 text-xs'>
            ...
            <p class='font-medium text-slate-700'>
                {{ course.trainer?.fullName || 'Nicht zugewiesen' }}
            </p>
        </div>
        <p v-if='course.comment' class='mt-3 text-sm text-slate-600'>
            {{ course.comment }}
        </p>
