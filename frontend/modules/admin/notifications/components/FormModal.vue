        ...
        <form class='space-y-4' @submit.prevent="emit('submit')">
            <UFormGroup label='Reichweite' :error='fieldErrors.courseIds'>
                <div class='mb-2 flex items-center gap-3'>
                    <UToggle v-model='form.isGlobal' data-testid='notification-global-toggle' />
            ...
            <UFormGroup label='Titel' :error='fieldErrors.title'>
                <UInput
                    ...
                    placeholder='Betreff'
                    required
                    @update:model-value="emit('clear-field-error', 'title')"
                />
            </UFormGroup>
            <UFormGroup label='Nachricht' :error='fieldErrors.message'>
                <UTextarea
                    ...
                    placeholder='Nachricht an die Kursteilnehmer…'
                    :rows='4'
                    required
                    @update:model-value="emit('clear-field-error', 'message')"
                />
            </UFormGroup>
            <UFormGroup label='Angepinnt bis' hint='Optional – Mitteilung wird bis zu diesem Datum oben angezeigt' :error='fieldErrors.pinnedUntil'>
                <UInput
                    v-model='form.pinnedUntil'
                    data-testid='notification-pinned-until'
                    type='date'
                    @update:model-value="emit('clear-field-error', 'pinnedUntil')"
                />
            </UFormGroup>
            <UAlert
                v-if='formError'
