    ...
    <UCard class="auth-card">
      ...
      <form class="space-y-4 min-w-0" @submit.prevent="handleRegister">
        <UFormGroup label="Name" :error="errorFor('name')">
          <UInput v-model="form.name" placeholder="Max Mustermann" required @update:model-value="clearFieldError('name')" />
        </UFormGroup>

        <UFormGroup label="E-Mail" :error="errorFor('email')">
          <UInput v-model="form.email" type="email" placeholder="name@beispiel.de" required @update:model-value="clearFieldError('email')" />
        </UFormGroup>

        <UFormGroup label="Passwort" :error="errorFor('password')">
          <UInput v-model="form.password" type="password" placeholder="Mindestens 6 Zeichen" required @update:model-value="clearFieldError('password')" />
        </UFormGroup>

        <UFormGroup label="Passwort bestätigen" :error="errorFor('confirmPassword')">
          <UInput v-model="confirmPassword" type="password" placeholder="Passwort wiederholen" required @update:model-value="clearFieldError('confirmPassword')" />
        </UFormGroup>
